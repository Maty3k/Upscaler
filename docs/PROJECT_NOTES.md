...
| — | **Recipes** (Batch tab op) + `upscaler recipe` — an ordered chain of adjust/effects/blur/sharpen/crop/watermark steps as JSON, with per-step regions (faces and depth resolved at run time) and output settings (format, size budget, strip metadata); unknown keys tolerated, unknown tools refused | `recipe.py` |
| — | **Remove metadata (privacy)** (Convert tab mode) + `upscaler metadata` — reports GPS/serial/thumbnail in plain language; strips by rewriting JPEG segments / PNG chunks so pixels are byte-identical; keeps the orientation tag so photos stay upright; inspect-only by default | `metadata.py` |
| — | **Fit a file-size budget** (Convert tab mode) + `upscaler optimize` — binary-searches encoder quality for the largest that fits, then estimates the resize from the overshoot; auto=WebP, strips metadata, skips files already under budget | `optimize.py` |
| — | **Design templates** (Design tab) — layers positioned in % of the canvas so one template renders at any size; palette roles (`accent`, `ink`, `role@80`) and font roles keep a template portable; text is fitted (binary-searched size + greedy wrap) rather than placed; templates are JSON | `design.py` |
| — | **Screenshot beautifier** (Screenshot tab) — chrome → rounded → tilt → spin → shadow → background → canvas; `result_size` drops every setting that can't move an edge so it stays cheap per slider move; `frame.perspective_coeffs` made public and panel's duplicate removed | `screenshot.py` |
| — | **Text behind the subject** (Watermark tab) — cut the subject out, lay the mark on the background, put the subject back on top; orthogonal to placement so tiled works too; cut-out cached in State so sliders don't re-run the model | `watermark.py` |
| — | **Watermark** tab + `upscaler watermark` — text or logo, 9 anchors or tiled-at-an-angle, relative sizing, outline+shadow defaults for legibility; font machinery split into `fonts.py` so it doesn't import torch | `watermark.py`, `fonts.py` |
| — | **Crop & Frame** tab + `upscaler crop` — aspect/custom-ratio crop (reuses fit.py), fill-vs-fit, straighten + keystone both trimmed to the largest all-real-pixel rect, exact size, border/rounded corners/shadow | `frame.py` |
| — | **Depth-of-field blur** (Blur tab region `depth`) — Depth Anything V2 Small on onnxruntime (quantised, ~26MB, Apache-2.0, sha-pinned); blur grows with distance from the focus by compositing 5 blur levels, not cross-fading; tap/click to focus | `depth.py`, `blur.py` |
